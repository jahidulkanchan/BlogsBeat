  ...
  useEffect(() => {
    const fetchAllBlogs = async () => {
      ...
      const { data } = await axios.get(
        ...
        }/all-blogs?category=${category}&search=${search}&page=${currentPage}&size=${itemPerPage}`
      );
      setBlogs(data);
      setLoading(false);
    };
      ...
      <section className="px-2 container mx-auto min-h-[510px] pt-[100px] md:pt-[120px] md:px-5">
        <div className="filter-box flex md:justify-center flex-wrap gap-5 md:gap-7">
          <div className="search max-w-[320px] flex items-center gap-2 pr-4 border bg-white w-fit">
            <div className="relative">
              <input
                ...
                type="text"
              />
              <div className="icon text-lg absolute top-3 right-2">
                <IoIosSearch />
              </div>
            </div>
          </div>
          <select
            ...
            className="p-2 border w-fit outline-none bg-white"
            name="category"
            id="category"
            value={category}
            required
        ...
        {blogs.length === 0 && (
          <p className="text-center text-xl text-secondary my-[100px]">
            Blogs Not Found
          </p>
        )}
        {blogs.length > 0 && (
          <div className="pagination flex justify-center gap-2 md:gap-5">
            ...
            {pages.map((page, i) => (
              <button
                ...
                className={`border border-primary text-primary hover:bg-darkPrimary duration-100 hover:text-white px-3 md:px-5 py-1 ${
                  currentPage === page ? "bg-darkPrimary text-white" : ""
                }`}
                key={i}
              >
                {page + 1}
